...
export class AccountResolvers {
  ...
  @Authorized()
  @Mutation(() => Account)
  async linkSocial (@Arg('type') type: SocialLinkType, @Arg('id', { nullable: true }) id: string, @Ctx() { address }: Context): Promise<DocumentType<Account>> {
    const account = await AccountModel.findOne({
      address,
    }).exec()
    ...
    return await AccountModel.findByIdAndUpdate(account, {
